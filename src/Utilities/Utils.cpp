...
namespace Utils {
    namespace Slot {
        RE::BGSEquipSlot* GetLeftHandSlot() {
            ...
            const REL::Relocation<func_t> func{ RELOCATION_ID(23'150, 23'607) };
            return func();
        }

        RE::BGSEquipSlot* GetVoiceSlot() {
            using func_t = decltype(&GetVoiceSlot);
            const REL::Relocation<func_t> func{ RELOCATION_ID(23'153, 23'610) };
            return func();
        }

        RE::BGSEquipSlot* GetRightHandSlot() {
            using func_t = decltype(&GetRightHandSlot);
            const REL::Relocation<func_t> func{ RELOCATION_ID(23'151, 23'608) };
            return func();
        }
        ...
        void CleanSlot(RE::PlayerCharacter* a_pc, RE::BGSEquipSlot* a_slot) {
            ...
            auto* dummy = RE::TESForm::LookupByID<RE::TESForm>(0x2'01'63)->As<RE::TESObjectWEAP>();
    ...
    namespace Time {
        float GGTM() {
            static auto g_SGTM = reinterpret_cast<float*>(RELOCATION_ID(511'883, 388'443).address());
            return *g_SGTM;
        }

        void SGTM(const float a_in) {
            static auto g_SGTM = reinterpret_cast<float*>(RELOCATION_ID(511'883, 388'443).address());
            *g_SGTM            = a_in;
            using func_t       = decltype(SGTM);
            REL::Relocation<func_t> func{ RELOCATION_ID(66'989, 68'246) };  //never used???
        }
    }  // namespace Time
    ...
    namespace Inventory {
        ...
        uint16_t GetNextUniqueID() {
            ...
            {
                const auto invc{ pc->GetInventoryChanges() };
                if(!invc) {
                    return 0;
                }
        ...
        Hand GetWeaponEquippedHand(const RE::Actor* a_actor, const RE::TESObjectWEAP* a_weapon, const uint32_t a_uniqueID, const bool itemClean) {
            if(!a_actor) {
                return Hand::None;
            }
            ...
            if(lhs && lhs->object && lhs->object->GetFormID() == a_weapon->GetFormID()) {
                if(lhs->extraLists) {
                    for(auto* extraList : *lhs->extraLists) {
                        if(!extraList->HasType(RE::ExtraDataType::kEnchantment) && !extraList->HasType(RE::ExtraDataType::kHealth) && !extraList->HasType(RE::ExtraDataType::kPoison)) {
        ...
        RE::InventoryEntryData* GetSelectedItemIninventory(RE::InventoryMenu* a_invMenu) {
            ...
            auto* il = a_invMenu->GetRuntimeData().itemList;
            if(!il) {
                return nullptr;
            }
            const RE::ItemList::Item* selectedItem = il->GetSelectedItem();
            if(!selectedItem) {
                return nullptr;
            }
        ...
        RE::TESForm* GetSelectedFormInMagicMenu(const RE::MagicMenu* a_magMen) {
            if(a_magMen) {
                RE::GFxValue result;
                ...
                if(result.GetType() == RE::GFxValue::ValueType::kNumber) {
                    const auto formID{ static_cast<RE::FormID>(result.GetNumber()) };
    ...
    void NotificationMessage(std::string_view a_message) {
...
void Utils::Magic::GetMagicItemDescription(RE::ItemCard* a_itemCard, RE::MagicItem* a_magicItem, RE::BSString& a_str) {
    ...
    const REL::Relocation<func_t> func{ RELOCATION_ID(51'022, 51'900) };
...
RE::NiMatrix3 Utils::Math::MatrixFromAxisAngle(const float theta, const RE::NiPoint3& axis) {
