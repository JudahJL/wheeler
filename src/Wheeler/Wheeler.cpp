...
void Wheeler::Update(const float a_deltaTime) {
    std::shared_lock lock{ _wheelDataLock };
    ...
    if(poppedUp) {
        ...
        DrawArgs drawArgs;
        drawArgs.alphaMultiplier = fadeLerp;
        ...
        for(int i{}; i < _wheels.size(); i++) {
            Drawer::draw_circle_filled(
                ...
                Config::Styling::Wheel::WheelIndicatorSize, i == _activeWheelIdx ? Config::Styling::Wheel::WheelIndicatorActiveColor : Config::Styling::Wheel::WheelIndicatorInactiveColor, 10, drawArgs);
        }
...
void Wheeler::Clear() {
    std::unique_lock lock(_wheelDataLock);
...
void Wheeler::OpenWheeler() {
    ...
    if(!pc || !pc->Is3DLoaded() || !ui) {
        return;
    }
    static constexpr std::array conflictingMenus{
        ...
        "LootMenu"sv  // quick loot
    };
    for(std::string_view menuName : conflictingMenus) {
        if(ui->IsMenuOpen(menuName) && !ui->IsMenuOpen(RE::InventoryMenu::MENU_NAME) && !ui->IsMenuOpen(RE::MagicMenu::MENU_NAME)) {
            return;
        }
    }
...
void Wheeler::UpdateCursorPosMouse(float a_deltaX, float a_deltaY) {
    ...
    ImVec2 newPos{
        _cursorPos + ImVec2{ a_deltaX, a_deltaY }
    };
    ...
    if(distanceFromCenter > cursorRadius) {
        ...
        const ImVec2 direction{ newPos / distanceFromCenter };
...
void Wheeler::ActivateHoveredEntrySecondary() {
    ...
    if(_state == WheelState::KOpened) {
        const auto& activeWheel{ _wheels[_activeWheelIdx] };
...
void Wheeler::DeleteCurrentWheel() {
    ...
    if(_wheels.size() > 1) {
        const auto& toDelete{ _wheels[_activeWheelIdx] };
...
void Wheeler::MoveWheelBack() {
    ...
    if(_wheels.size() > 1) {
        ...
        if(_activeWheelIdx == 0) {
            ...
            _wheels.emplace_back(std::move(currentWheel));
            _activeWheelIdx = _wheels.size() - 1;
        } else {
            targetIdx = _activeWheelIdx - 1;
            std::swap(_wheels[_activeWheelIdx], _wheels[targetIdx]);
...
void Wheeler::SetActiveWheelIndex(const int a_index) {
...
void Wheeler::SerializeFromJsonObj(const nlohmann::json& j_wheeler, SKSE::SerializationInterface* a_intfc) {
    const auto& j_wheels{ j_wheeler["wheels"] };
    for(const auto& j_wheel : j_wheels) {
        auto wheel{ Wheel::SerializeFromJsonObj(j_wheel, a_intfc) };
        _wheels.emplace_back(std::move(wheel));
    }
...
void Wheeler::SerializeIntoJsonObj(nlohmann::json& j_wheeler) {
    ...
    for(const std::unique_ptr<Wheel>& wheel : _wheels) {
        ...
        j_wheeler["wheels"].emplace_back(std::move(j_wheel));
    }
    j_wheeler["activewheel"] = _activeWheelIdx;
}

void Wheeler::SetupDefaultWheels() {
    constexpr int defaultWheelNum{ 2 };
    constexpr int defaultEntryNum{ 4 };
    Wheeler::Clear();
    int wheelIdx{};
...
inline ImVec2 Wheeler::getWheelCenter() {
    using namespace Config::Styling::Wheel;
    return { ImGui::GetIO().DisplaySize.x / 2 + CenterOffsetX, ImGui::GetIO().DisplaySize.y / 2 + CenterOffsetY };
}
