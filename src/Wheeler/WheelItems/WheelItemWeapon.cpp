...
void WheelItemWeapon::DrawSlot(const ImVec2 a_center, RE::TESObjectREFR::InventoryItemMap& a_imap, const DrawArgs& a_drawArgs) {
    const int         itemCount{ this->GetItemExtraDataAndCount(a_imap).first };
    const std::string text{ std::format("{}{}", this->_obj->GetName(), itemCount > 1 ? std::format(" ({})", itemCount) : "") };

    WheelItemWeapon::drawSlotText(a_center, text.data(), a_drawArgs);
    this->drawSlotTexture(a_center, a_drawArgs);
}

void WheelItemWeapon::DrawHighlight(const ImVec2 a_center, RE::TESObjectREFR::InventoryItemMap& a_imap, const DrawArgs& a_drawArgs) {
    WheelItemWeapon::drawHighlightText(a_center, this->_obj->GetName(), a_drawArgs);
    ...
    if(!descriptionBuf.empty()) {
        WheelItemWeapon::drawHighlightDescription(a_center, descriptionBuf.data(), a_drawArgs);
    }
...
WheelItemWeapon::WheelItemWeapon(RE::TESBoundObject* a_weapon, uint16_t a_uniqueID) {
    ...
    auto iconType{ Texture::icon_image_type::sword_one_handed };
...
void WheelItemWeapon::ActivateItemSecondary() {
    ...
    bool isTwoHanded{ this->_obj->As<RE::TESObjectWEAP>()->IsCrossbow() || this->_obj->As<RE::TESObjectWEAP>()->IsBow() || this->_obj->As<RE::TESObjectWEAP>()->IsTwoHandedSword() || this->_obj->As<RE::TESObjectWEAP>()->IsTwoHandedAxe() };
...
void WheelItemWeapon::ActivateItemPrimary() {
    ...
    const auto equippedHand{ Utils::Inventory::GetWeaponEquippedHand(pc, this->_obj->As<RE::TESObjectWEAP>(), this->GetUniqueID(), true) };
...
void WheelItemWeapon::equipItem(bool a_toRight) const {
    try {
        const auto pc{ RE::PlayerCharacter::GetSingleton() };
        if(!pc || !pc->Is3DLoaded()) [[unlikely]] {
            return;
        }
        RE::TESObjectREFR::InventoryItemMap inv{ pc->GetInventory() };
        auto [count, extraData]{ this->GetItemExtraDataAndCount(inv) };
...
void WheelItemWeapon::unequipItem(const RE::BGSEquipSlot* a_slot) const {
    try {
        auto pc{ RE::PlayerCharacter::GetSingleton() };
...
bool WheelItemWeapon::IsAvailable(RE::TESObjectREFR::InventoryItemMap& a_inv) {
    const auto [first, _]{ this->GetItemExtraDataAndCount(a_inv) };
