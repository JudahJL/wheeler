...
void WheelItemArmor::DrawSlot(ImVec2 a_center, RE::TESObjectREFR::InventoryItemMap& a_imap, const DrawArgs& a_drawArgs) {
    ...
    WheelItemArmor::drawSlotText(a_center, text.data(), a_drawArgs);
    this->drawSlotTexture(a_center, a_drawArgs);
}

void WheelItemArmor::DrawHighlight(ImVec2 a_center, RE::TESObjectREFR::InventoryItemMap& a_imap, const DrawArgs& a_drawArgs) {
    WheelItemArmor::drawHighlightText(a_center, this->_obj->GetName(), a_drawArgs);
    this->drawHighlightTexture(a_center, a_drawArgs);

    // get inventory entry data, ptr -> unique_ptr(dangerous)
    auto*                   armor{ this->_obj->As<RE::TESObjectARMO>() };
    ...
    WheelItemArmor::drawHighlightDescription(a_center, descriptionBuf.data(), a_drawArgs);
    ...
    WheelItemArmor::drawItemHighlightStatIconAndValue(a_center, this->_stat_texture, armorRating, a_drawArgs);
}
...
WheelItemArmor::WheelItemArmor(RE::TESBoundObject* a_armor, uint16_t a_uniqueID) {
    ...
    // get armor texture
    auto* armor{ a_armor->As<RE::TESObjectARMO>() };
    using SlotEnum = RE::BGSBipedObjectForm::BipedObjectSlot;
    auto iconType{ Texture::icon_image_type::armor_default };
...
void WheelItemArmor::equipArmor() const {
    const auto [pc, aeMan]{
    ...
    }
    {
        ...
        const auto [count, extraData]{ this->GetItemExtraDataAndCount(inv) };
        if(count <= 0) {  // nothing to equip
            return;
        }
        const auto slot{ this->_obj->As<RE::TESObjectARMO>()->GetEquipSlot() };
        aeMan->EquipObject(pc, _obj, extraData, 1, slot);
    }
}

void WheelItemArmor::unequipArmor() const {
    const auto [pc, aeMan]{
        std::pair{ RE::PlayerCharacter::GetSingleton(), RE::ActorEquipManager::GetSingleton() }
    };
    if(!pc || !pc->Is3DLoaded() || !aeMan) [[unlikely]] {
...
bool WheelItemArmor::IsActive(RE::TESObjectREFR::InventoryItemMap& a_inv) {
    ...
    const auto entry = a_inv.find(this->_obj)->second.second.get();
    return entry && entry->IsWorn();
}

bool WheelItemArmor::IsAvailable(RE::TESObjectREFR::InventoryItemMap& a_inv) {
    const auto [fst, _]{ this->GetItemExtraDataAndCount(a_inv) };
