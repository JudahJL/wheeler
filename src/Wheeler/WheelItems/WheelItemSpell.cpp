...
void WheelItemSpell::DrawSlot(const ImVec2 a_center, RE::TESObjectREFR::InventoryItemMap&, const DrawArgs& a_drawArgs) {
    WheelItemSpell::drawSlotText(a_center, this->_spell->GetName(), a_drawArgs);
    this->drawSlotTexture(a_center, a_drawArgs);
}

void WheelItemSpell::DrawHighlight(const ImVec2 a_center, RE::TESObjectREFR::InventoryItemMap&, const DrawArgs& a_drawArgs) {
    WheelItemSpell::drawHighlightText(a_center, this->_spell->GetName(), a_drawArgs);
    ...
    if(!descriptionBuf.empty()) {
        WheelItemSpell::drawHighlightDescription(a_center, descriptionBuf.data(), a_drawArgs);
    }
}

bool WheelItemSpell::IsActive(RE::TESObjectREFR::InventoryItemMap&) {
    ...
    // power can also be equipped to lhs or rhs, so we're checking them as well.
    {
        const auto lhs{ pc->GetEquippedObject(true) };
        lhsEquipped = lhs && lhs->GetFormID() == this->_spell->GetFormID();

        const auto rhs{ pc->GetEquippedObject(false) };
        rhsEquipped = rhs && rhs->GetFormID() == this->_spell->GetFormID();
    }
    return lhsEquipped || rhsEquipped || powerEquipped;
}

bool WheelItemSpell::IsAvailable(RE::TESObjectREFR::InventoryItemMap&) {
    ...
    if(pc && aeMan) [[likely]] {
        if(this->isPower()) {
            if(pc->GetActorRuntimeData().selectedPower && pc->GetActorRuntimeData().selectedPower->GetFormID() == this->_spell->GetFormID()) {
                aeMan->UnequipObject(pc, this->_spell->As<RE::TESBoundObject>(), nullptr, 2);
                aeMan->UnequipObject(pc, this->_spell->As<RE::TESBoundObject>(), nullptr, 1);
                aeMan->UnequipObject(pc, this->_spell->As<RE::TESBoundObject>(), nullptr, 0);
            } else {
...
void WheelItemSpell::ActivateItemSpecial() {
    // return;  // current don't do anything because I'm yet to figure out how to prevent the power from being cast when it shouldn't
    // auto pc = RE::PlayerCharacter::GetSingleton();
    // if(!pc) {
    //     return;
    // }
    // if(this->_spell->data.flags.any(RE::SpellItem::SpellFlag::kInstantCast) || this->_spell->GetSpellType() == RE::MagicSystem::SpellType::kPower || this->_spell->GetSpellType() == RE::MagicSystem::SpellType::kLesserPower) {  // insta cast the spell or power
    //
