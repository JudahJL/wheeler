...
namespace Hooks {
    namespace {
        ...
        private:
            ...
            void Hook_PickUpObject(RE::TESObjectREFR* a_object, uint32_t a_count, bool a_arg3, bool a_playSound) {
                ...
                const auto bo{ a_object->GetBaseObject() };
                if(!bo) {
                    return _PickUpObject(this, a_object, a_count, a_arg3, a_playSound);
                }
                    ...
                    for(auto* entry : *invChanges->entryList) {
                        ...
                        if(item == a_object->GetBaseObject()) {
                            ...
                            if(entry->extraLists) {
                                for(auto* xList : *entry->extraLists) {
                                    if(xList) {
                                        const auto count2{ xList->GetCount() };
                                        rawCount -= count2;
                                        UniqueIDHandler::EnsureXListUniqueness(xList);
                                    }
                                }
    ...
    public:
        static void Install() {
            REL::Relocation<std::uintptr_t> hook1{ RELOCATION_ID(49'852, 50'784) };  // 84AB90, 876700

            auto& trampoline{ SKSE::GetTrampoline() };
            _TESCamera_Update = trampoline.write_call<5>(hook1.address() + RELOCATION_OFFSET(0x1'A6, 0x1'A6), TESCamera_Update);  // 84AD36, 8768A6
        }
