	...
	_original(originalValue)
{
    ...
    const std::function<void()> funcReset = [this]() { // tell the interpolator to interpolate back to the original value once it's reached the target.
...
void TimeBounceInterpolator::SetValue(const double value)
{
	_interpolator.SetValue(value);
}

void TimeBounceInterpolator::ForceValue(const double value)
{
