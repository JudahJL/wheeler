...
constexpr auto STYLESETTINGS_PATH{ R"(Data\SKSE\Plugins\wheeler\Styles.ini)" };
constexpr auto CONTROLSETTINGS_PATH{ R"(Data\SKSE\Plugins\wheeler\Controls.ini)" };

bool GetBoolValue(const CSimpleIniA& ini, const char* section, const char* key, bool& value) {
    auto strValue{ ini.GetValue(section, key, nullptr) };
    if(strValue != nullptr) {
...
bool GetUInt32Value(const CSimpleIniA& ini, const char* section, const char* key, uint32_t& value) {
    ...
    if(strValue != nullptr) {
        try {
            ...
            logger::error("Failed to parse {}: {} when reading uint32 value.", section, key);
            return false;
        }
    }
    return false;
...
bool GetFloatValue(const CSimpleIniA& ini, const char* section, const char* key, float& value) {
    ...
    if(strValue != nullptr) {
        try {
            ...
            logger::error("Failed to parse {}: {} when reading float value.", section, key);
            return false;
        }
    }
    return false;
