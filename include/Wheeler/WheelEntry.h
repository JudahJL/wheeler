...
public:
    ...
    /// (GTA5-style)
    /// </summary>
    void DrawBackGround(ImVec2 wheelCenter, ImVec2 entryCenter,
                        float entryInnerAngleMin, float entryInnerAngleMax,
                        float entryOuterAngleMin, float entryOuterAngleMax,
                        bool hovered, int numArcSegments, RE::TESObjectREFR::InventoryItemMap& a_imap, const DrawArgs& a_drawARGS);
    ...
    /// </summary>
    void DrawSlotAndHighlight(ImVec2 a_wheelCenter, ImVec2 a_entryCenter, bool a_hovered, RE::TESObjectREFR::InventoryItemMap& a_imap, const DrawArgs& a_drawArgs);

    /// <summary>
    /// Get the radius changes made by arcRadiusIncInterpolator. Use this function to calculate the offset of item center.
    /// </summary>
    /// <returns></returns>
    float GetRadiusMod() const;
    ...
    void PrevItem();
    void NextItem();
    void PushItem(const std::shared_ptr<WheelItem>& item);
...
private:
    void drawSlot(ImVec2 a_center, bool, RE::TESObjectREFR::InventoryItemMap& a_imap, const DrawArgs& a_drawArgs);
    void drawHighlight(ImVec2 a_center, RE::TESObjectREFR::InventoryItemMap& a_imap, const DrawArgs& a_drawArgs);
    ...
    TimeBounceInterpolator _arcRadiusBounceInterpolator{ 0 };  // for animating the arc radius's bouncing when the entry is clicked
};
