...
public:
    ...
    void EmplaceEntry(std::unique_ptr<WheelEntry>& a_entry);
	void PushEmptyEntry();

    void PrevItemInHoveredEntry() const;
    void NextItemInHoveredEntry() const;
    ...
    /// </summary>
    void ActivateHoveredEntryPrimary(bool a_editMode) const;
	...
	void ActivateHoveredEntrySpecial(bool a_editMode) const;
	void SetHoveredEntryIndex(int a_index);

	void MoveHoveredEntryForward();
	void MoveHoveredEntryBack();

    void SerializeIntoJsonObj(nlohmann::json& a_json) const;
