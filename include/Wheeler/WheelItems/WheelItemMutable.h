...
protected:
     WheelItemMutable() = default;
    ~WheelItemMutable() override;

    /// <summary>
    /// Returns the count and extradatalist of "this*" item in the inventory.
    ...
    ///
    /// Note: One ambiguous behavior that I can't fix is when the user adds a pair of items(counts as one item) to the wheel
    ...
    std::pair<int, RE::ExtraDataList*> GetItemExtraDataAndCount(RE::TESObjectREFR::InventoryItemMap& a_inv) const;

    void GetItemEnchantment(RE::TESObjectREFR::InventoryItemMap& a_invMap, std::vector<RE::EnchantmentItem*>& r_enchantments) const;
    void GetItemEnchantment(const RE::InventoryEntryData* a_iData, std::vector<RE::EnchantmentItem*>& r_enchantments) const;

    RE::TESBoundObject* _obj{ nullptr };  // TESObjectWEAP or TESObjectARMO
