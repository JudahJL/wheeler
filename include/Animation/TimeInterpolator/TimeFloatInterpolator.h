...
public:
     TimeFloatInterpolator() = default;
    ~TimeFloatInterpolator();

    TimeFloatInterpolator(const double initialValue, const std::function<void()>& callback) :
    value(initialValue), target(initialValue)
    {
        _callbacks.push_back(callback);
    }

    explicit TimeFloatInterpolator(const double initialValue):
        value(initialValue), target(initialValue) { }
...
////we have class `TimeFloatInterpolator` and a manager class `TimeFloatInterpolatorManager`, which is also a static class. `TimeFloatInterpolator`'s constructor puts itself into the manager class' storage, which is a std:unordered_set<TimeFloatInterpolator*>; TimeFloatInterpolator's destructor removes itself from the manager class.
////
